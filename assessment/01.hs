...
index n xs 
    | null xs = -1
    | n == head xs = 0
    | otherwise = if i < 1 then -1
                  else i
    where i = 1+(index n (tail xs))
-- 2 ----------
indexFold n xs = if i == (length xs) then -1 else i
    where i = foldl (\x y -> if snd x then x else (if y == n then (fst x, True) else ((fst x) + 1, False))) (0,False) xs
-- 3 ----------
...
toothpicks xs = map (\x -> ((head x)!!0, (head x)!!1,(head x)!!2)) $ group $ sort [sort [x,y,z] | x <- xs, y <- delete x xs, z <- delete y (delete x xs), triangle x y z]

toothpicksGather xs = [((x!!0),(x!!1),(x!!2)) | x <- list, triangle (x!!0) (x!!1) (x!!2)]
    where list = gather xs
...
main = do
    ...
    print $ toothpicks [1,5,6,7]
    print $ toothpicks [1,2,5,6,7]
    print $ toothpicksGather [2,3,5,12,15,2]
