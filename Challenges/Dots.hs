...
neib :: [a] -> Int -> Int -> [a]
neib xs index lim = take (up - down) $ drop down xs
  where
    up = index + lim + 1
    down = max (index - lim) 0
...
-- getHoles :: String -> [Int]
getHoles xs = filter (> -1) $ map (\x -> if xs !! x == '.' then x else -1) list
  where
    list = [0 .. length xs -1]
...
fitTest :: IO ()
fitTest = do
  -- (fit ("12.345", 1) 2) se torna uma função curry faltando só uma var que seria o valor recebido do vetor
